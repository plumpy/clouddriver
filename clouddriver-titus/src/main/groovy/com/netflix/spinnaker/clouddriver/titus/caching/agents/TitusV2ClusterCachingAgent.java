...
public class TitusV2ClusterCachingAgent
    implements CachingAgent, CustomScheduledAgent, OnDemandAgent {
  ...
  public TitusV2ClusterCachingAgent(
    ...
    this.metricsSupport =
        new OnDemandMetricsSupport(
            registry, this, TitusCloudProvider.ID + ":" + OnDemandType.ServerGroup);
  ...
  public String getAgentType() {
    return account.getName()
        ...
        + region.getName()
        + "/"
        + TitusV2ClusterCachingAgent.class.getSimpleName();
  }
  ...
  public OnDemandResult handle(ProviderCache providerCache, Map<String, ?> data) {
    ...
    PercentileTimer.get(registry, metricId.withTag("operation", "handleOnDemand"))
   ...
   *
   * <p>By writing a minimal record to ON_DEMAND only, we eliminate significant overhead (redis and
   * network) at the cost of an increase in time before a change becomes visible in the UI.
   *
   * <p>A change will not be visible until a caching cycle has completed.
   */
  private OnDemandResult onDemand(ProviderCache providerCache, Job job, Map<String, ?> data) {
    String serverGroupKey =
        Keys.getServerGroupV2Key(
            data.get("serverGroupName").toString(), account.getName(), region.getName());
    ...
    return new OnDemandResult(
        getOnDemandAgentType(), new DefaultCacheResult(cacheResults), evictions);
  }
  ...
  public CacheResult loadData(ProviderCache providerCache) {
    ...
    PercentileTimer.get(registry, metricId.withTag("operation", "getAllJobsWithoutTasks"))
        .record(System.currentTimeMillis() - startTime, MILLISECONDS);
    ...
    PercentileTimer.get(registry, metricId.withTag("operation", "getTaskIdsForJobIds"))
        .record(System.currentTimeMillis() - startJobIdsTime, MILLISECONDS);

    List<String> serverGroupKeys =
        jobs.stream()
            .map(
                job -> Keys.getServerGroupV2Key(job.getName(), account.getName(), region.getName()))
    ...
    List<String> pendingOnDemandRequestKeys =
        providerCache
            .filterIdentifiers(
                ON_DEMAND.ns,
                Keys.getServerGroupV2Key("*", "*", account.getName(), region.getName()))
            .stream()
    ...
    pendingOnDemandRequestsForServerGroups.forEach(
        onDemandEntry -> {
          if (Long.parseLong(onDemandEntry.getAttributes().get("cacheTime").toString()) < startTime
              && Long.parseLong(
                      onDemandEntry.getAttributes().getOrDefault("processedCount", "0").toString())
                  > 0) {
    ...
    Map<String, CacheData> onDemandMap =
        keepInOnDemand.stream().collect(Collectors.toMap(CacheData::getId, it -> it));
    List<String> evictFromOnDemandIds =
  ...
  /**
   * Used to build a cache result, whether normal or on demand, by transforming known Titus objects
   * into Spinnaker cached objects
   */
            ...
            .map(
                job -> {
                          ...
                          .filter(
                              it ->
                                  it.getJobId().equalsIgnoreCase(job.getId())
                                      && cacheablePolicyStates.contains(
                  ...
                  List<String> jobLoadBalancers =
                      allLoadBalancers.getOrDefault(job.getId(), Collections.emptyList());
                  return new ServerGroupData(
                      job,
                      jobScalingPolicies,
                      jobLoadBalancers,
                      taskAndJobIds.get(job.getId()),
                      account.getName(),
                      region.getName());
                })
            .collect(Collectors.toList());
